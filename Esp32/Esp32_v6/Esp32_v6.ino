...
const char *serverUrl = "http://192.168.43.182:3001/upload/firmware32.bin";
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "garden/rain/number") {
    numberrain = messageTemp.toInt();
        EEPROM.write(3, numberrain);
        EEPROM.commit();
    Serial.println(numberrain);
  }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("esp32Client")) {
      ...
      client.subscribe("garden/rain/number");
...
void Number() {
  ...
  if (now - lastMeasure > 3000) {
    ...
    String setRain = String(EEPROM.read(3));
    setRain.toCharArray(Rainn, setRain.length() + 1);
    //
    ...
    client.publish("garden/rain/number", Rainn);
